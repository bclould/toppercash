...
#if defined(NDEBUG)
# error "toppercash cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "toppercash Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // toppercash: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// toppercash: find block wanted by given orphan block
...
// toppercash: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // toppercash: target change every block
    // toppercash: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal toppercash miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // toppercash: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // toppercash: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // toppercash: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // toppercash: track money supply and mint amount info
...
// toppercash: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // toppercash: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // toppercash: compute stake entropy bit for stake modifier
    ...
    // toppercash: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // toppercash: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // toppercash: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// toppercash: attempt to generate suitable proof-of-stake
...
{
    RenameThread("toppercash-loadblk");
