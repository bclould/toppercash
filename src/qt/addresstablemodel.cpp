...
{
    // Update address book model from toppercash core
...
QString AddressTableModel::pubkeyForAddress(const QString &address) const
{
    CPubKey pubkey;
    CKeyID keyid;
    CBitcoinAddress(address.toStdString()).GetKeyID(keyid);
    if (walletModel->getPubKey(keyid, pubkey))
    {
        return  QString("%1").arg(HexStr(pubkey).c_str());
    }
    return QString("");
}
...
QString AddressTableModel::privkeyForAddress(const QString &addr) const
{
    ...
    if (!address.SetString(addr.toStdString()))
        return  QObject::tr("Invalid toppercash address");
    if (fWalletUnlockStakingOnly)
        return  QObject::tr("Wallet is unlocked for staking only.");
    CKeyID keyID;
    if (!address.GetKeyID(keyID))
        return  QObject::tr("Address does not refer to a key");
    CKey vchSecret;
    if (!wallet->GetKey(keyID, vchSecret))
        return  "Private key for address " + addr + " is not known";
    return QString("%1").arg(CBitcoinSecret(vchSecret).ToString().c_str());
}
