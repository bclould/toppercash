...
static const int64_t MAX_MONEY = 96000000 * COIN;
inline bool MoneyRange(int64_t nValue) { return (nValue >= 0 && nValue <= MAX_MONEY); }
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const unsigned int LOCKTIME_THRESHOLD = 500000000; // Tue Nov  5 00:53:20 1985 UTC
static const int64_t IPO_SHARE = 96000000 * COIN;
...
public:
    ...
    uint8_t hasData;
    std::vector<unsigned char>   sData;//附件的数据
    ...
    CTransaction(int nVersion, unsigned int nTime, const std::vector<CTxIn>& vin, const std::vector<CTxOut>& vout, unsigned int nLockTime)
        : nVersion(nVersion), nTime(nTime), vin(vin), vout(vout), nLockTime(nLockTime), nDoS(0),hasData(0)
    {
    }
    ...
    {
        // toppercash: the coin stake transaction is marked with the first output empty
...
public:
    ...
    // toppercash: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // toppercash: two types of block: proof-of-work or proof-of-stake
    ...
    // toppercash: get max transaction timestamp
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // toppercash: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // toppercash: block index flags
    enum  
    {
